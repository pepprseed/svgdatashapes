...
def example8( dispmode='curves' ):
    ...
    p.rectangle( x='D', height='all', color='#ffc', adjust=(0,-35,0,0) )
    ...
    for row in plotdata:
        ...
        if dispmode == 'curves':
            ...
            for key in [ 't1', 't2', 't3', 't4' ]:
                ...
                p.datapoint( x=key, y=row[key], diameter=5, color='#b44' )

        elif dispmode in ['bars', 'updown']:
            barcolor = [ 'orange', 'blue', 'green', 'red' ]
            icolor = 0
            for key in [ 't1', 't2', 't3', 't4' ]:
                p.bar( x=key, y=row[key], width=5, color=barcolor[icolor], ybase=ybase )
                icolor += 1
