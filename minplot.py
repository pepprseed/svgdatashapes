...
def textprops( ptsize=None, color=None, opacity=None, anchor=None, rotate=None, adjust=False, cssclass=False, cssstyle=False, reset=False ):
...
def lineprops( width=None, color=None, opacity=None, dash=False, cssclass=False, cssstyle=False, reset=False ):
    ...
    if reset == True or cssclass != False:           # cssclass .... note False vs. None; user can specify  cssclass=None
        if cssclass == False: cssclass = None
        p_line["class"] = cssclass

    if reset == True or cssstyle != False:           # cssstyle .... note False vs. None; user can specify  cssstyle=None
        if cssstyle == False: cssstyle = None
        p_line["style"] = cssstyle
...
def txt( x, y, txt, anchor=None ):
    ...
    if p_text["rotate"] != 0:
        p_svg["out"] += "transform=\"rotate(" + str(p_text["rotate"]) + " " + str2f(x) + "," + str2f(flip(y)) + ")\" "
    len0 = len(txt)
...
def datacolumns( namelist=None ):
    # namelist is an array or list
    global p_colnames
    p_colnames = namelist
    return True
...
def numinfo( datarows=None, numcol=None, distrib=False, distbinsize=None, accumcol=None, percentiles=False ):
    ...
    if distrib == True:   # run a freq distribution
        ...
        if accumcol != None: tfindex = getdfindex( accumcol, datarows )
...
def getdfindex( colname=None, datarows=None ):
    ...
    if datarows == None:  raise AppError( "expecting datarows= " )
        ...
        try: str = datarows[0][colname]       # test to see if colname is a valid dict member
    ...
    try:  dfindex = int( colname )     # test for an int colname eg. "2"
        ...
        if p_colnames == None: raise AppError( "datarows= are not dict and data column names have not been defined, see datacolumns()" )
        try:  dfindex = p_colnames.index( colname )
        except: raise AppError( "data column name '" + str(colname) + "' not recognized" )
    return dfindex
